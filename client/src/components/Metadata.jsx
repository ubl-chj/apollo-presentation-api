...
export class Metadata extends React.Component {
  ...
  buildItemList (data) {
    return (<div className="Hj59Ib">
      ...
      <ul>
        {data.manifest ? data.manifest.metadata.map(
          (metadata) => <MetadataItem key={uuidv4()} language={defaultLang} label={metadata.label}
  ...
  render () {
    ...
    if (manifestId) {
      return (<Query query={Metadata.query()} variables={{manifestId}}>
        {({loading, error, data}) => {
          if (loading) {
            return <p>Loading...</p>
          }
