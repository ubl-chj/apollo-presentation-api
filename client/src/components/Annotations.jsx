...
export class Annotations extends React.Component {
  ...
  render () {
    ...
    if (manifestId && canvasId && annotationPageId) {
      return (<Query query={Annotations.query()} variables={{manifestId, canvasId, annotationPageId}}>
        {({loading, error, data}) => {
          if (loading) {
            return <p>Loading...</p>
          }
          ...
          return (<div>
            ...
            {data.annotationPage ? data.annotationPage.items.map(
              (c) => <AnnotationsItem manifestId={manifestId} canvasId={canvasId} annotationPageId={annotationPageId}
                id={c.id} type={c.type} motivation={c.motivation} target={c.target} body={c.body}
                key={uuidv4()}/>) : 'no annotations returned from endpoint'}
          </div>)
        }}
      </Query>)
    } else {
