...
export class Canvases extends React.Component {
  ...
  render () {
    ...
    if (manifestId) {
      return (<Query query={Canvases.query()} variables={{manifestId}}>
        {({loading, error, data}) => {
          if (loading) {
            return <p>Loading...</p>
          }
          ...
          return (<div>
            ...
            {data.manifest ? data.manifest.items.map(
              (c) => <CanvasItem manifestId={manifestId} id={c.id} type={c.type} label={c.label} width={c.width}
                height={c.height} key={uuidv4()}/>) : null}
          </div>)
        }}
      </Query>)
    } else {
