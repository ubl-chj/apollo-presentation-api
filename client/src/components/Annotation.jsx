...
export class Annotation extends React.Component {
  ...
  static query() {
    return gql`
          query Summary($manifestId: String!, $canvasId: String!, $annotationPageId: String!, $annotationId: String!) {
  ...
  render () {
    ...
    if (manifestId && canvasId && annotationPageId && annotationId) {
      return (
        <Query query={Annotation.query()} variables={{manifestId, canvasId, annotationPageId, annotationId}}>
          {({loading, error, data}) => {
            if (loading) {
              return <p>Loading...</p>
            }
