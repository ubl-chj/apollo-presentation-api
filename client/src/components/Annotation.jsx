...
export class Annotation extends React.Component {
  ...
  render () {
    ...
    if (manifestId && canvasId && annotationPageId && annotationId) {
      return (<Query query={Annotation.query()} variables={{manifestId, canvasId, annotationPageId, annotationId}}>
        {({loading, error, data}) => {
          if (loading) {
            return <p>Loading...</p>
          }
          ...
          return (<div>
            ...
            {data.annotation
              ? <AnnotationItem manifestId={manifestId} id={data.annotation.id} type={data.annotation.type}
                motivation={data.annotation.motivation} target={data.annotation.target} body={data.annotation.body}
                key={uuidv4()}/> : null}
          </div>)
        }}
      </Query>)
